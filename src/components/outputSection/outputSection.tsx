...
const VitalCard = ({ label, value, unit, status }: any) => {
  ...
  return (
    <div
      className={`rounded-2xl p-4 flex flex-col justify-between border transition-all hover:scale-105 ${
        isAbnormal
...
const OutputSection: React.FC<OutputSectionProps> = ({ isVisible = true }) => {
  ...
  return (
    // UPDATED: w-full and NO max-width to fill the container
    <div className="w-full text-slate-900 animate-in fade-in duration-700">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-6 gap-4">
        <div>
          ...
          <p className="text-slate-500 text-sm font-medium ml-8 mt-1">
            Generated in 1.2s • Model Confidence 94%
          </p>
        </div>
        <div className="flex gap-3">
          <button className="px-5 py-2.5 rounded-xl bg-white/40 border border-white/60 text-sm font-semibold text-slate-600 hover:bg-white hover:text-blue-600 transition-all shadow-sm">
            Export PDF
          </button>
          <button className="px-5 py-2.5 rounded-xl bg-blue-600 text-sm font-semibold text-white shadow-lg shadow-blue-500/30 hover:bg-blue-700 transition-all">
            Save to EHR
          </button>
        </div>
      </div>

      {/* Main Grid - w-full ensures it stretches */}
      <div className="w-full grid grid-cols-1 lg:grid-cols-12 gap-6">
        
        {/* --- COL 1: Diagnosis & Vitals (Left Side - Spans 8 cols) --- */}
        <div className="lg:col-span-8 flex flex-col gap-6 h-full">
          
          {/* 1. Primary Diagnosis Hero Card */}
          <div className={`${glassStyle} p-8 relative overflow-hidden group min-h-[220px] flex flex-col justify-between`}>
            ...
            <div className="flex justify-between items-start">
              <div className="flex items-start gap-5">
                <div className="w-16 h-16 rounded-2xl bg-red-50 border border-red-100 flex items-center justify-center shadow-sm shrink-0">
            ...
            <div className="bg-white/40 rounded-xl p-5 border border-white/50 mt-6">
        ...
        {/* --- COL 2: Action Plan & Risks (Right Side - Spans 4 cols) --- */}
        <div className="lg:col-span-4 flex flex-col gap-6 h-full">
          
          {/* 3. Clinical Plan Checklist */}
          <div className={`${glassStyle} p-6 flex flex-col h-full min-h-[300px]`}>
            ...
            <div className="flex-1 space-y-3">
              {analysisData.plan.map((item, i) => (
                <div
                  key={i}
                  className="group flex items-center justify-between p-3 rounded-xl bg-white/30 hover:bg-white/60 border border-transparent hover:border-blue-100 transition-all cursor-pointer"
                >
            ...
            <button className="mt-6 w-full py-3 rounded-xl border-2 border-dashed border-slate-300/60 text-slate-400 text-sm font-semibold hover:border-blue-400 hover:text-blue-500 hover:bg-white/40 transition-all flex items-center justify-center gap-2">
          ...
          <div className={`${glassStyle} p-6`}>
            ...
            <div className="bg-amber-50 rounded-xl p-4 border border-amber-100">
              ...
              <ul className="list-disc list-inside text-xs text-amber-800/80 space-y-1 font-medium ml-1">
